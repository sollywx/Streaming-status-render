...
class ModClient extends Client {
  ...
  log() {
    ...
    const logMessages = {
      ...
      no: `[+] READY : [${this.user.tag}] Premium user in the Miyako server`
        .gray,
    };
...
(async () => {
  ...
  if (!Array.isArray(users)) {
    console.warn(
      "Warning: 'users' is not an array. Wrapping it into an array."
    );
    users = [users];
  }
  ...
  console.clear();
  console.log(
    "[+] STREAMING : 2.1.4ccc - 18:10 10/8/2024 | deobf version".blue
  );
